...
//             <FcGoogle />
//             <button onClick={()=>login()} className="text-white font-semibold px-5">
...
//         </div>
//         <div className="flex justify-center">
//           <button onClick={()=>handleSubmit()} className="text-black font-semibold rounded-full bg-logoColor  space-x-3 px-8 py-3 w-80">
//             Login
//           </button>
//         </div>
...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    .then((response) => {
      ...
      navigate('/');
      setLog(true);
      setUserData(userData); // Use userData from response
    })
    .catch((error) => {
      toast.error(error.response.data.messages);
      console.error("login error", error);
